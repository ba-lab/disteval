...
def get_args():
    ...
    parser.add_argument('--visualize',                                         action="store_true",
        help='Create heatmap visualizations.')
    parser.add_argument('--color_scheme', type=int, action='store', default = 1, choices = {1, 2, 3},
        help="Color scheme for visualization. 1: linear gradient (default), 2: semantic coloring, 3: perceptually uniform."
        )
    parser.add_argument('--vis_filetype', type=str, action='store', default = 'svg', choices = {'svg', 'png', 'jpg'},
        help="File type for visualization files. (svg or png or jpg)")
...
## ######### ADDED BY JAMIE - Visualizations

if args.visualize:
    # recalculate to place pearson on scatter plot
    if D is not None and ND is not None:
        seq_len = len(D)
        pearson_8 = calc_dist_errors(D, ND, seq_len, dist_thres = 8, min_sep = 12, top_l_by_x = .0001, pred_limit = 20)["pearsonr"]
        pearson_20 = calc_dist_errors(D, ND, seq_len, dist_thres = 20, min_sep = 12, top_l_by_x = .0001, pred_limit = 20)["pearsonr"]
    else:
        pearson_8 = 0
        pearson_20 = 0
    import disteval_visualization
    disteval_visualization.do_visualization(args, native_basename, basename, ND, D, NC, C, pearson_8, pearson_20)

## ######### DONE WITH ADDITION BY JAMIE - Visualizations
